...
class CornerFacade(Button):
	...
	def in_boundaries(self, position):
		if (math.sqrt((self.center[0]-position[0])**2
		              + (self.center[1]-position[1])**2) < self.radius):
			return True
		else:
			return False
	...
	def draw(self):
		if (self.corner.ownership != 0):
			self.rect = pygame.draw.circle(self.screen, 
			                               self.player_to_colour_mapping[
										    self.corner.ownership-1], 
										  self.center, self.radius,0)
			if self.corner.settlement == "city":
				self.rect = pygame.draw.circle(self.screen, 
				                               self.colour.get("BLACK"),
											   self.center, 
											   round(self.radius/2), 3)
		self.rect = pygame.draw.circle(self.screen, self.colour.get("BLACK"),
		                               self.center, self.radius,3)
