...
class BoardFacade:
	...
	def __generate_facades(self, start = [320.0, 320.0], size = 70.0):
		...
		corners_of_tile = []
		for e in self.tile_facades[9].tile.edges:
			for c in e.corners:
		...
		for c in corners_of_tile:
			...
			cf = CornerFacade([round(center_x), round(center_y)], c, self.screen)
			self.corner_facades.append(cf)
