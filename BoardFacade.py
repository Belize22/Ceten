...
class BoardFacade:
    ...
    def generate_tile_facades(self, size):
        ...
        for lt in land_tiles:
            self.land_tile_facades.append(
                LandTileFacade(self.screen, lt, size))
        for st in sea_tiles:
            self.sea_tile_facades.append(SeaTileFacade(self.screen, st, size))

    """generate_edge_facades:
    Each edge facade is generated by iterating through all the tile
    facades, retrieving the tile's vertices, looping through the
    tile clockwise and giving the edge facade the two endpoints of
    the edge.
    """
    ...
    """generate_corner_facades:
    Each corner facade is generated by iterating through all the tile
    facades and retrieving the tile's vertices. Each vertex represents
    a corner and the screen coordinates for those represent the center
    of the facade.
    """
    ...
    """render_control_menu:
    Render all the controls that involve direct interaction with the
    Ceten board.
    """
    ...
    def place_robber(self, mouse_pos, player_facade):
        ...
        if tile_facade is not None:
            robber_tile_facade = self.find_robber_tile()
    ...
    def end_maritime_trade(self, player_facade):
        self.board.advance_game_phase()
        self.allow_maritime_trade_if_possible(player_facade)
        self.end_turn_button.enabled = True
        self.end_turn_button.draw()
    ...
    def end_turn(self, player_facade):
        ...
        if current_phase_before == CurrentPhase.VICTORY_PHASE.value:
            ...
            self.current_feedback = (
                    player_facade.player.retrieve_player_name()
                    + " has won Ceten.")
        else:
    ...
    def find_robber_tile(self):
        tile = self.board.find_robber_tile()
        for tf in self.land_tile_facades:
            if tf.tile == tile:
                return tf
    ...
    def update_dice_value(self, roll):
        pygame.draw.circle(
    ...
    Ceten board, alongside the proper feedback to ensure the
